        ...
        val peticionDatosCovid = JsonArrayRequest(Request.Method.GET.url,null,Response.Listener { response ->
            for (index in 0..response.length()-1){
                try {
                    ...
                    val nombrePais = paisJson.getString("conuntreregion")
                    val numoeroConfirmados = paisJson.getInt("Confirmed")
                    var numeroMuertos = paisJson.getInt("deaths")
                    val numeroRecuperados = paisJson.getInt("recovered")
                    val countryCodeJson = paisJson.getJSONEObject("countrycode")
                    ...
                    Log.wtf("JsonError".LocalizedMessage)
                }
            }
